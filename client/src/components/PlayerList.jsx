...
import ProfileModal from "./ProfileModal"; // Re-use your modal!
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50">
      <div className="p-4 bg-gray-800 text-white font-bold text-lg flex justify-between items-center">
        ...
        <span className="text-xs bg-gray-700 px-2 py-1 rounded">Room Host: {players.find(p => p.id === hostId)?.name || "?"}</span>
      </div>
        ...
        {players.map((player) => (
          <div 
            ...
            className={`flex items-center justify-between p-2 rounded-lg border-2 transition ${
              player.id === currentUserId ? "border-blue-400 bg-blue-50" : "border-gray-200 bg-white"
            }`}
          >
            ...
            >
              <div className="w-10 h-10 rounded-full bg-gray-200 overflow-hidden border">
                 {/* Fallback avatar if seed missing */}
                 <img src={`https://api.dicebear.com/9.x/pixel-art/svg?seed=${player.name}`} alt="av" />
              </div>
              <div>
                ...
                <p className="text-xs text-gray-500 font-mono">{player.score} pts</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              {/* Ready Status */}
              {player.isReady ? (
                <span className="text-green-500 font-bold text-xs border border-green-200 bg-green-50 px-2 py-1 rounded-full">READY</span>
              ) : (
                 !player.isHost && <span className="text-gray-400 text-xs">...</span>
              )}
      ...
      {/* Re-use Profile Modal for ANY user clicked */}
      <ProfileModal 
